...
void DesfireReaderComponent::publish_nonauthorised_uid_(const char *uid_str) {
  if (!nonauthorised_uid_sensor_) return;
  if (strcmp(uid_str, last_nonauthorised_uid_) != 0) {
    size_t len = strlen(uid_str);
    if (len >= sizeof(last_nonauthorised_uid_)) len = sizeof(last_nonauthorised_uid_) - 1;
    memcpy(last_nonauthorised_uid_, uid_str, len);
    last_nonauthorised_uid_[len] = '\0';
    nonauthorised_uid_sensor_->publish_state(last_nonauthorised_uid_);
  }
}

void DesfireReaderComponent::note_nonauthorised_failure_() {
  if (nonauthorised_fail_count_ < 255)
    nonauthorised_fail_count_++;

  ESP_LOGD(TAG, "Nonauthorised failure count for current card: %u",
           nonauthorised_fail_count_);

  if (nonauthorised_fail_count_ >= 2 && current_uid_str_[0] != '\0')
...
void DesfireReaderComponent::start_select_app_() {
    ...
    if (ack_ok) {
      ...
      while (millis() - start < 100) {
        if (try_read_response_(0x12, rf_resp, sizeof(rf_resp), rf_len)) {
          ESP_LOGCONFIG(TAG, "PN532 MaxRetries configured.");
          break;
        }
        delay(2);
      }
...
void DesfireReaderComponent::loop() {
  ...
  case NfcState::DETECT_WAIT_RESP: {
    ...
    if (try_read_response_(PN532_CMD_IN_LIST_PASSIVE, resp, sizeof(resp), resp_len)) {
      if (resp_len > 0 && resp[0] != 0) {
        ...
        // New card
        bool same_uid = (uid_len > 0 && uid_ptr != nullptr &&
                         uid_len == current_uid_len_ &&
                         memcmp(current_uid_bytes_, uid_ptr, uid_len) == 0);
  ...
  case NfcState::AUTH1_WAIT_RESP: {
    ...
    if (read_desfire_apdu_(resp1, sizeof(resp1), resp1_len, sw1, sw2)) {
      if (sw2 != DESFIRE_MORE_FRAMES || resp1_len != 16) {
        ESP_LOGE(TAG, "Auth1 bad response (sw2=0x%02X, len=%d)", sw2, resp1_len);
        handle_retry_();
        return;
      }
  ...
  // ─────────────────────────────────────────────
  case NfcState::RELEASE_WAIT_ACK: {
    if (try_read_ack_()) {
      state_ = NfcState::RELEASE_WAIT_RESP;
      state_entered_at_ = millis();
      return;
    }
    if (millis() - state_entered_at_ > ACK_TIMEOUT_MS) {
  ...
  // ─────────────────────────────────────────────
  case NfcState::RELEASE_WAIT_RESP: {
    uint8_t resp[4];
    uint8_t resp_len;
